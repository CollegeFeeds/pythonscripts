...
cursor2=db.cursor()
#cursor3=db.cursor()
cmd="""select * from api_hansrajcounters"""
cursor2.execute(cmd)
...
for i in soup2:
     ...
     #print "1"
     if title==alt[1]:
         cursor2.execute("""update api_hansrajcounters set hansrajtitle=%s""",(titlef,))
...
while len(m) !=0:
     r=m.pop()
     p=k.pop()
     sql="""insert into api_hansrajnotices(id,title,linkf) values(NULL,'%s','%s')"""%(p,r)
     cursor2.execute(sql)
     if condi == 1:     
         cursor2.execute("""update api_hansrajcounters set hansrajtitle=%s""",(p,))

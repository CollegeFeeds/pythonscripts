...
cursor2=db.cursor()
cmd="""select * from api_datediplomacounters"""
cursor2.execute(cmd)
...
href=soup2
print soup2
while soup2!=last_notification and j<27:
	#print soup2
	try:
		link=soup2["href"]
	...
	sql="""insert into api_datediploma(id,link,title) values(NULL,'%s','%s')"""%(link,name)
	cursor2.execute(sql)
cursor2.execute("""update api_datediplomacounters set link=%s""",(href,))
db.commit()
db.close()
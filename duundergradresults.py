...
cursor2=db.cursor()
#cursor3=db.cursor()
cmd="""select * from api_undergradcounters"""
cursor2.execute(cmd)
...
for i in soup2:
     ...
     if title == alt[1]:
         cursor2.execute("""update api_undergradcounters set undergradtitle=%s""",(titlef,))
         break 
     m.append(link)
     k.append(title)
     titlef=title
...
while len(m) !=0:
     r=m.pop()
     p=k.pop()
     sql="""insert into api_undergradresults(id,title,linkf) values(NULL,'%s','%s')"""%(p,r)
     cursor2.execute(sql)
     if condi == 1:
         cursor2.execute("""update api_undergradcounters set undergradtitle=%s""",(p,))
     counter=counter+1

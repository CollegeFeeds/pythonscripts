...
cursor2=db.cursor()
cmd="""select * from api_ugdatetheorycounters"""
cursor2.execute(cmd)
...
while soup2!=last_notification and j<106:
	...
	sql="""insert into api_ugdatetheory(id,link,title) values(NULL,'%s','%s')"""%(link,name)
	cursor2.execute(sql)
cursor2.execute("""update api_ugdatetheorycounters set link=%s""",(href,))
db.commit()
db.close()